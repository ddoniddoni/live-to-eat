...
문서 작성 시점에는 앱, DB migration, 실제 Takeout 파서, API 연동, 스토어 빌드를 실행하지 않았다. 아래 항목은 앞으로 충족할 출시 조건이다. 체크하지 않은 일을 완료로 보고하지 않는다.
...
| T02 | A/B/비회원/정지/탈퇴 계정의 RLS와 RPC 권한, service role 우회 경로 차단, 개인 메모/태그/가져온 원문/정확한 방문일/숨긴 저장 수 비노출 | R05,R09,R12 |
| T03 | 검색→저장→종료→재접속, 중복 버튼, 재시도, 동시 수정 version 충돌, 폴더 중복 소속, 삭제 후 목록/지도 일치 | R01,R02,R03,R15 |
...
| T06 | 동의받은 실제 Takeout의 비식별 fixture, CSV BOM/따옴표/개행/메타데이터, 다른 헤더 매핑, ZIP 폭탄/경로 탈출/중첩/암호화/허용 크기 초과와 잘못된 JSON | R13,R15 |
| T07 | 가져오기 200행 이상, 같은 장소 여러 목록, 원래 메모 보존, 동명이점 수동 선택, 미확인/비음식 장소, 부분 성공, 취소/재실행, 만료된 batch, 이미 저장된 장소 재처리 | R13,R15 |
| T08 | Google 단일 링크 붙여넣기/짧은 링크, SSRF 우회/리다이렉트 제한, iOS 정상 Share Extension, Android ACTION_SEND, 앱 실행 전/후/로그아웃/중복 수신 | R14 |
...
| T15 | Places field mask/호출 수/쿼터, 익명 API 비용 공격, 토큰/주소/메모 로그 유출, Google 캐시/백업 삭제 수명, 장애 시 kill switch, 데이터 복구 연습 | R02,R09,R13,R17,R20 |
| T16 | 작은 화면/큰 글씨/VoiceOver/TalkBack, ko/en 번역 누락, 저사양 기기에서 500개 기록/지도 점진 로딩, 두 스토어 배포 빌드에서 핵심 흐름 회귀 검증 | R01,R11,R17,R19,R20 |

P0 blocker: 개인 정보 누출, 다른 계정 수정, 잘못된 장소 대량 확정, 중복 대량 저장, 철회 링크 계속 열림, API 키 오용, 앱 실행/로그인/저장/지도/공유 크래시. 성능 목표는 M0 기준 기기에서 계측값으로 정한다. 실제 네트워크 API 시간을 제외한 화면/파서 지연도 따로 기록한다. 성능 수치를 측정하지 않고 60fps/대량 처리 보장을 마케팅에 쓰지 않는다.
...
| 베타 | TestFlight와 Play internal/closed 테스트에서 production에 가까운 환경으로 T01~T16 실행. 적용 대상인 신규 개인 Play 계정은 최소 12명의 테스터가 14일 연속 참여한 closed test 등 production 접근 신청 요건 확인. 모든 계정에 동일한 조건이라고 쓰지 않는다. [L7] |
...
Google Takeout 이전은 “내보낸 파일을 선택해서 가져오기”로 정확히 설명한다. 지원되지 않는 계정 연결/실시간 동기화를 스크린샷이나 광고에 넣지 않는다. “Google 공식 앱/공식 제휴”를 암시하는 이름/로고도 사용하지 않는다.
...
P0 권한은 현재 위치 버튼의 foreground 위치, OS 파일 선택, 공유 확장에 필요한 최소 범위로 한다. 백그라운드 위치/연락처/통화 기록/사진 전체 접근은 요청하지 않는다. Takeout 전체 ZIP은 서버에 보내지 않는다. 위치를 자체 DB에 저장하지 않더라도 Google SDK/API에 전달되는 데이터가 있을 수 있으므로 “위치가 절대 기기 밖으로 나가지 않음”으로 고지하지 않는다.

크래시/분석 도구를 추가하면 이벤트에 private 메모, 원본 URL, 주소, 좌표, Takeout 파일명/내용, 공유 secret, access/refresh token을 담지 않는다. 세션 녹화/화면 캡처는 P0 비활성이다. 오류 수집은 build/platform/error code/request ID 중심으로 하고 실제 제3자 전송 필드를 검사한다. SDK를 추가한 뒤 Data safety/App Privacy 검토도 다시 한다.

탈퇴 플로우: 최근 본인 재인증 → 서버에서 계정 deleting 전환/모든 공개 접근과 쓰기 차단 → 세션 철회 및 provider revoke 작업 → 사용자 데이터/비밀/가져오기 임시 행 삭제 → Supabase Auth 사용자 삭제 → 완료 상태/재시도 감사. Apple revoke 등 후속 작업에 필요한 식별/철회 정보는 공개 데이터와 분리해 최소 범위로 안전하게 유지하고 성공 후 제거한다. Supabase Auth 행부터 지워 필요한 철회 정보를 잃지 않는다. [L9]
...
운영 기본값(법정 기간이 아니라 변경 가능한 제품 설정): importer 임시 행/기기 공유 inbox 최대 7일, 민감값을 제거한 진단 로그 30일, 최소한의 운영 보안 감사 90일. 실제 필요한 기간과 법적 검토 결과를 확정하고 TTL job/검사를 구현한다. 탈퇴/취소 시 기본 TTL보다 먼저 제거해야 하는 데이터는 즉시 처리한다.
...
장애 runbook은 04의 작업 기록 안에 실제 수행 결과를 남긴다. API 비용 폭증이면 Places 검색/가져오기 기능 flag와 서버 rate limit으로 신규 호출을 제한하고 개인 저장 기록은 보존한다. 지도 장애는 오류/목록으로 대체한다. 보안 사고라면 API/공유/계정 접근 범위를 차단하고 키/토큰 회전, 영향 평가, 필요한 고지와 복구를 담당자가 수행한다. 가격 알림은 자동 지출 차단 장치가 아니다.
...
EAS Update는 호환되는 runtime의 JS/asset 변경에 사용한다. 지도 SDK, native module, Share Extension, App Group, 권한, config plugin, OS 연결 설정이 바뀌면 새 네이티브 빌드가 필요하다. runtimeVersion 정책을 명시하고 preview channel에서 검증 후 production으로 승격한다. [L11]
...
우선 계측할 값은 `첫 10곳 저장 완료율`, `Takeout 중 확인 후 저장/중복/미확인 비율`, `지역 링크 생성→비회원 열람→타인 저장 전환`, `7일/30일 재방문`, `사용자 1명/가져오기 1건당 실제 인프라 비용`이다. 실험 표본과 분모를 명시하며 본인 직접 열람/봇을 전환에서 제외한다.
...
동의 기반의 최소 이벤트만 수집하고, 개인 저장 장소 전체를 분석 서비스에 보내지 않는다. 예를 들어 import 결과는 행 수/결과 코드, 공유 전환은 난수 이벤트 ID와 생성/열람/저장 상태로 집계한다. 장소 취향은 민감한 생활 패턴을 추론할 수 있으므로 공개 기본값을 수익화 목적으로 바꾸지 않는다.

## 9. 공개 직전 최종 게이트

- [ ] R01~R20 범위와 T01~T16의 실제 결과가 기록되어 있고 blocker가 없다.
- [ ] 양 플랫폼 production 서명 빌드에서 Google 지도/Google와 Apple 로그인/저장/가져오기/공유/탈퇴가 작동한다.
- [ ] 동의받은 실제 Takeout 파일로 지원 포맷을 검증했고, 앱 설명이 실제 범위와 일치한다.
- [ ] iOS 정상 공유 확장과 Android 공유 수신을 실제 Google Maps에서 검증했다.

...
| 내 지도·목록·지역 필터 (B08) | PARTIAL | 공통 지역 선택, 지역/방문/폴더/보기/정렬과 둘러보기 지역의 기기 복원을 구현하고 Android 재실행을 검증했다. 기존 지도 핀/클러스터·목록 source의 자체 DB 전환과 DB 적용·실계정·iOS 검증은 미실행 |
| 공개 지도·둘러보기·타인 저장 (B12) | PARTIAL | 국내 체험에서는 선택 지역의 실제 공개 장소로 사람 목록·장소 수·상세를 필터링한다. 공개 지도 DTO와 private 재저장 source는 있으나 실제 지역 검색 계약·DB 적용·Edge 배포·교차계정 검증은 미연결 |
...
| 품질·접근성·출시 점검 (B16) | PARTIAL | 단위 테스트 87건·번역 614키, Android QA 앱의 정상/실패·재시도·화면 설정 복원과 100%/200% 글씨 검증, 양 플랫폼 JS bundle 확인. 이전 모의 인증 상태 검사 9건·브라우저 검사 기록을 유지하며 실제 연결·iOS 네이티브·한국어 IME·VoiceOver/TalkBack 음성·실기기·스토어 검증은 남음 |

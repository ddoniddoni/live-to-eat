...
사용자가 아래 네 작업의 계획 수립과 순차 구현을 요청했다. 1~2번의 연동 전 구현을 마쳤으며 2번의 iOS 네이티브·스크린리더 음성 검증은 별도다. 3~4번은 계획 상태다. Supabase·DB·API·지도 SDK 연결, 실제 데이터 적재와 운영 설정은 계속 보류한다.
...
| 3 | 실패와 재시도 / B07,B08,B13,B16 / R01,R03,R08,R17 | 연결 끊김·시간 초과·저장 실패·재시도·중복 요청, 요청 중/실패 뒤 작성값 유지 | 모의 응답으로 실패→수정/재시도 성공, 중복 저장 방지, 늦은 응답·화면 종료 시 안전성 확인. 실제 서버 권한·오프라인 동기화 완료로 표시하지 않음 | PLANNED |
...
| 품질·접근성·출시 점검 (B16) | PARTIAL | 단위 테스트 49건·번역 553키, Android QA 앱 빌드와 Maestro 핵심 흐름 100%/200% 글씨 검증, 양 플랫폼 JS bundle 확인. 이전 모의 인증 상태 검사 9건·브라우저 검사 기록을 유지하며 iOS 네이티브·한국어 IME·VoiceOver/TalkBack 음성·실기기·스토어 검증은 남음 |

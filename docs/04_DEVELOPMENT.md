...
사용자가 아래 네 작업의 계획 수립과 순차 구현을 요청했다. 1~3번의 연동 전 구현을 마쳤으며 iOS 네이티브·스크린리더 음성과 실제 서버 검증은 별도다. 4번은 계획 상태다. Supabase·DB·API·지도 SDK 연결, 실제 데이터 적재와 운영 설정은 계속 보류한다.
...
| 4 | 도움말·정책 문서 열람 / B06,B15 / R12,R18,R19 | FAQ·문의 진입·약관/개인정보 문서 열람 구조와 돌아오기·문서 연결 실패 안내 | 한영 열람/탐색/오류 상태 확인. 운영 연락처·공개 URL·법적 본문/버전은 소유자 확정 후 연결하며 임의 완성 문서로 게시하지 않음 | PLANNED |
...
`apps/mobile/.maestro`의 세 흐름은 예시 장소 저장·재편집·지역 선택·공유 초안 생성, 프로필 입력 포커스·작성 유지/버리기, 신고 사유 유지·검토에서 돌아오기·작성 버리기를 검사한다. Android에서는 소프트 키보드가 열린 상태의 첫 뒤로가기, 공유·신고의 이전 단계 이동, 확인창 취소, 프로필 탭에서 내 지도로 돌아오기도 확인한다. 실제 계정/서버를 사용하지 않는다.

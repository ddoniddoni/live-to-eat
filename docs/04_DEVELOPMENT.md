...
| 내 지도·목록·지역 필터 (B08) | PARTIAL | 공통 계층형 지역 탐색·검색·적용/취소, 지역·미분류 필터와 탭 이동 시 지역 선택 유지를 구현했다. 지도 핀/클러스터·목록 동기화 source는 있으나 이전 외부 상세/refresh quota를 자체 DB 조회로 전환해야 한다. DB 적용·실계정/실기기 검증은 미실행 |
...
| 품질·접근성·출시 점검 (B16) | PARTIAL | 단위 테스트 64건·번역 563키, Android QA 앱의 정상/실패·재시도 흐름과 100%/200% 글씨 검증, 양 플랫폼 JS bundle 확인. 이전 모의 인증 상태 검사 9건·브라우저 검사 기록을 유지하며 실제 연결·iOS 네이티브·한국어 IME·VoiceOver/TalkBack 음성·실기기·스토어 검증은 남음 |
